...
## [2026-05-04] lint | Weekly health check; 1 index-drift fix; 3 items flagged for human review
- script run: `bin/wiki_lint.py` — exit 1; 1 issue (index drift: anymailfinder missing from index)
- pages updated: [[index]] — added [[wiki/entities/anymailfinder]] under new "Agentic workflow tools (referenced)" section
- report: `LINT_REPORT.md` at repo root — 3 items flagged: Skool/communities tension, Google Anti-gravity unverified claim, Khairallah 18-month-window staleness check
- post-fix lint: clean (0 broken, 0 ambiguous, 0 orphans, 0 index drift, 0 frontmatter issues)
- notes: structurally clean wiki; 61 stubs are expected given the 2-day-old completionist-coverage policy.
