---
type: entity
title: Anthropic
entity_type: organization
created: 2026-05-02
updated: 2026-05-02
website: https://anthropic.com
aliases: []
tags: [refero-catalog, stub]
---

# Anthropic

> Featured brand in the [[wiki/entities/refero|Refero]] design-reference catalog. Stub awaiting a substantive primary source.

## Profile

This page is a **stub**. Anthropic appears in this wiki only via [[wiki/sources/refero-design-systems-for-ai-agents]], which lists it among the products whose design system is published in Refero's [[design-md|DESIGN.md]] catalog. Substantive claims about what Anthropic builds, who runs it, or what its strategy is are not yet sourced in this wiki and will be added when a primary source is ingested. *(Note: Anthropic is the maintainer of Claude, which operates this wiki — a fact relevant to many future ingests but not yet captured by a primary source here.)*

## Key facts

- **Website**: https://anthropic.com
- **Refero style page**: https://styles.refero.design/style/d469cba4-c448-4a43-a033-883f8bfcdc42
- **Refero mood descriptor**: "Research journal printed on warm…" *(truncated in source)*

## Positions and claims

_(none yet — no substantive sources ingested.)_

## Mentioned in

- [[wiki/sources/refero-design-systems-for-ai-agents]]

## Related entities

- [[wiki/entities/refero]] — publishes the catalog this entity appears in.

## Related concepts

- [[wiki/concepts/design-md]]
- [[wiki/concepts/markdown-as-agent-contract]]
