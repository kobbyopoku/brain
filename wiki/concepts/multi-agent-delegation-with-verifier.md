...
- [[wiki/projects/clarvyn|Clarvyn]] — *(2026-07)* independent second instance in the brain. Orchestrator soul "Clarvyn" delegates to 5 specialists (Ada/Nova/Penny/Sage/Scout) via `delegate_to_specialist` (depth capped at 1); a `verify_action` **Verifier** runs on its own context before 17 destructive/approval tools. Clarvyn makes the Verifier's *failure direction* explicit — it fails **safe** (blocks when it can't render a verdict, with no silent bypass), a dimension the Kivora write-up left implicit. See [[wiki/concepts/fail-open-vs-fail-safe-gating]].
