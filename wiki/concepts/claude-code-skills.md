...
- [[wiki/sources/nexu-io-open-design]] — *2026-05-05*. The **largest substantive skill-pack** in the wiki — [[wiki/entities/open-design|Open Design]]'s `skills/` directory contains **64 SKILL.md bundles** spanning prototypes (web/mobile/dashboard/landing), HTML decks (21 deck flavors including pitch decks, weekly reports, magazine posters), documents (PRDs, runbooks, OKRs, finance reports), media generation (HyperFrames HTML→MP4, Seedance text-to-video, image posters, audio jingles), and meta-skills (critique, tweaks, fidelity audit). Bundles [[wiki/entities/guizang-ppt-skill|guizang-ppt-skill]] verbatim with attribution. The wiki now has three substantive non-Anthropic skill-packs in size order: Open Design (64) > Refero design-md (33) > marketingskills (139 tactics packed across fewer files).
